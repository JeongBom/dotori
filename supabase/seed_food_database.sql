-- food_database 시드 (setup_all.sql ⑦에서 추출)
-- 프로덕션에 food_database가 비어 있어 장보기 체크 시 음식/생필품 자동 분류가 동작하지 않음.
-- Supabase SQL Editor에서 이 파일 전체를 실행하면 됨. (name UNIQUE + ON CONFLICT라 여러 번 실행해도 안전)
-- 테이블이 아예 없다면 setup_all.sql의 ④ CREATE TABLE food_database 블록을 먼저 실행할 것.
